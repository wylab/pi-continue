...
function sourceLabel(source: ContinuationEventSource): string {
	...
	if (source === "absolute-threshold") return "automatic compaction (absolute threshold)";
...
export function startContinuationCompaction(
	...
	// Pi's ctx.compact() disconnects and aborts the active operation before it
	// prepares the compaction. Calling ctx.abort() first lets Pi's automatic
	// compaction race the package-owned request and can leave the latter with an
	// "Already compacted" error, so this pipeline has one abort owner: ctx.compact().
	...
	try {
		ctx.compact({
			...
			onError: (error: Error) => {
				if (!claimCompactionCallback()) return;
				// Residual race (documented in RACE-RESEARCH.md): Pi's native threshold
				// compaction can win during ctx.compact()'s abort -> waitForIdle gate, so the
				// package compact throws "Already compacted" AFTER session_compact already
				// verified this event's proof. That is a benign loss: the package-owned
				// handoff was saved by the native winner. Complete instead of failing -
				// dispatchIfReady then dispatches the resume exactly once. Fail closed for
				// any other error, or for "Already compacted" without verified proof.
				const pending = runtime.pendingResumeDispatch;
				const proofVerified = pending?.eventId === event.id && pending.proofVerified;
				const alreadyCompacted = error?.message === "Already compacted";
				if (alreadyCompacted && proofVerified && isActiveRunningContinuationEvent(runtime, event.id)) {
					runtime.compactionRunning = false;
					runtime.guardFailureKey = undefined;
					markContinuationCompactionComplete(ctx, runtime, event.id);
					return;
