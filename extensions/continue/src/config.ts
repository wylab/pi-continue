...
/**
 * Policy: any session compacts at roughly 120-150k tokens regardless of the active model's
 * context window (user ruling 2026-08-19, superseding the scale-free window-relative formula).
 * Midpoint 130000 chosen inside the approved band; documented in PROVENANCE.md.
 * null disables the absolute-threshold trigger and veto.
 */
const DEFAULT_ABSOLUTE_COMPACT_THRESHOLD_TOKENS = 130_000;
...
export const DEFAULT_CONTINUE_CONFIG: ContinuationConfig = {
	...
	absoluteCompactThresholdTokens: DEFAULT_ABSOLUTE_COMPACT_THRESHOLD_TOKENS,
...
function parsePartialConfig(value: unknown): PartialContinuationConfig {
	...
	const absoluteCompactThresholdTokens = asNullableNumber(value.absoluteCompactThresholdTokens);
	if (absoluteCompactThresholdTokens !== undefined) result.absoluteCompactThresholdTokens = absoluteCompactThresholdTokens;
...
function normalizeConfig(partial: PartialContinuationConfig): ContinuationConfig {
	return {
		...
		absoluteCompactThresholdTokens: normalizeTokenOverride(partial.absoluteCompactThresholdTokens),
