...
function overThresholdCompactionEvent() {
...
test("below-threshold, manual, and opted-out compactions keep Pi's own summarizer", async () => {
	...
	try {
		...
		// No finished turn yet: this is what a manual /compact looks like.
		...
		// A /compact that carries instructions is a deliberate request.
		await completeAssistantTurn(pi, ctx);
		const instructed = overThresholdCompactionEvent();
		instructed.customInstructions = "focus on the failing test";
		assert.equal(await pi.events.get("session_before_compact")(instructed, ctx), undefined);

		await completeAssistantTurn(pi, ctx);
		assert.equal(await pi.events.get("session_before_compact")(compactionEvent(), ctx), undefined);
...
test("a concurrent compaction does not reuse the in-flight handoff owner", async () => {
	...
	try {
		...
		while (!releaseFirstAttempt) await new Promise((resolve) => setTimeout(resolve, 5));
		releaseFirstAttempt();
