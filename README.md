...
Adoption claims the single checkpoint opened when an assistant turn ends on its own, and only the first over-threshold compaction can claim it. These compactions stay with Pi's native summarizer:

- a `/compact` that carries instructions, or any compaction that does not directly follow a finished assistant turn
- a compaction Pi runs while submitting new user input, because that submission already drives the next turn
- a cancelled turn, and context-overflow recovery, which Pi retries itself
- any adopted checkpoint whose ledger cannot be built; it is handed back to Pi instead of cancelling the compaction

Pi does not tell extensions why it started compacting, so a bare `/compact` run in the seconds right after a finished turn can still claim that checkpoint. Set `adoptNativeCompaction` to `false` to keep every Pi-initiated compaction native.
